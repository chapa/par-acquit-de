...
pub fn validate_value<'v>(value: &str) -> form::Result<'v, ()> {
    if value.len() < 4 {
        return Err(FormError::validation("L'expression doit faire au moins 4 caractères"))?
    }

    if !value.starts_with("co") || value.chars().nth(2).filter(|c| c == &'n' || c == &'m').is_none() {
        return Err(FormError::validation("L'expression doit commencer par co et doit avoir comme troisième lettre m ou n"))?
    }

    if value.chars().nth(2).unwrap() == value.chars().nth(3).unwrap() {
        return Err(FormError::validation("Les deuxième et troisième lettre doivent être différentes"))?
    }
...
impl Data {
    ...
    pub fn get_word(&self, word: &str) -> Result<&Word, Error> {
        self.words.get(word.to_lowercase().as_str()).ok_or(Error::ThereIsNoWord)
    }
}
